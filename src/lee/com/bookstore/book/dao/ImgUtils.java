...
public class ImgUtils {
    ...
    public void test() {
        try {
            FileInputStream in = new FileInputStream("C:\\Users\\KingIsHappy\\Pictures\\icon.jpg");
            String out = "E:\\hhh.PNG";
            scale(in, out, 100, 100, true);
     ...
     *
     * @param in        要缩放的图片的输入流
     * @param ouputPath 缩放后的图片路径
     * @param height    目标高度像素
     * @param width     目标宽度像素
     * @param bb        是否补白
     */
    public final static void scale(InputStream in, String ouputPath, int height, int width, boolean bb) {
        try {
            ...
            ImageIO.write((BufferedImage) itemp, "JPG", new File(ouputPath));      //输出压缩图片
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
